...
function referralLog($candidateName,$candidateId,$companyName,$companyId) {
	$string="<span>[".getCurrentTimestamp()."]</span> $candidateName (Candidate-ID: $candidateId) was referred to $companyName (Company-ID: $companyId).\n";
...
function placementLog($candidateName,$candidateId,$companyName,$companyId) {
	$string="<span>[".getCurrentTimestamp()."]</span> $candidateName (Candidate-ID: $candidateId) was placed in $companyName (Company-ID: $companyId).\n";
	$fileData = file_get_contents("../../admin/history/log.txt");
	file_put_contents("../../admin/history/log.txt",$string.$fileData);
}

/* This function logs in when a candidate has been rejected. */
function rejectionLog($candidateName,$candidateId,$companyName,$companyId) {
	$string="<span>[".getCurrentTimestamp()."]</span> $candidateName (Candidate-ID: $candidateId) was rejected by $companyName (Company-ID: $companyId).\n";
	$fileData = file_get_contents("../../admin/history/log.txt");
	file_put_contents("../../admin/history/log.txt",$string.$fileData);
}
